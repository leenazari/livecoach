...
export async function DELETE() {
  try {
    ...
    try {
      ...
      console.error("Google disconnected but sender reconciliation failed", error?.message || error);
      warning = "Google was disconnected. Refresh Settings to confirm the remaining sender.";
    }
    const { error: auditError } = await supabaseService
      ...
      .insert({
        ...
        source: "human",
        action: "google_connector_disconnected",
    ...
    if (auditError)
      console.error("Google disconnect audit failed", auditError.message);
    ...
    return NextResponse.json(
      { error: error?.message || "Google could not be disconnected" },
