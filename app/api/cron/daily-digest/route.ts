...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [callsRes, completedRes, openRes, upcomingRes] = await Promise.all([
      supabaseAdmin
    ...
    for (const result of [callsRes, completedRes, openRes, upcomingRes]) {
    ...
    const companyIds = Array.from(
      new Set(
        [...calls, ...completed, ...(openRes.data || []), ...tomorrowCalls]
    ...
    const attentionRows = openTasks
      .filter((task: any) => {
        if (!task.due_at) return false;
        const due = new Date(task.due_at).getTime();
        return Number.isFinite(due) && due <= tomorrowEnd;
      })
      .sort((a: any, b: any) =>
    ...
    const attention = attentionRows.map((task: any) => {
      ...
      const company = companyNames.get(task.company_id);
      return `<strong style="color:${overdue ? "#a34b35" : "#9a7b12"};">${overdue ? "Overdue" : "Due soon"}</strong>: ${esc(task.text)}${company ? ` (${esc(company)})` : ""}`;
    });
    ...
    const sent = await sendMail({
      to: RECIPIENT,
      subject: `LiveCoach daily snapshot: ${when}`,
      html,
    });
...
function renderEmail(data: {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#fff;border:1px solid #e3e0da;border-radius:10px;"><tr><td style="padding:28px 30px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;color:#1c1b19;">
    ...
    <h1 style="margin:0 0 6px;font-size:22px;">Your daily progress snapshot</h1>
    <p style="margin:0 0 25px;font-size:14px;color:#777168;">${esc(data.when)} · ${data.callCount} calls captured · ${data.completedCount} tasks completed</p>
    ${section("Today’s progress", list(data.progress, "No completed calls or checked-off tasks were captured today."))}
    ${section("Bring to your attention", list(data.attention, "Nothing overdue or due in the next two days."))}
