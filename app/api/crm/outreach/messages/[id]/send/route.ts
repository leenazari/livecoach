...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!isReply && (["replied", "qualified", "not_interested", "suppressed"].includes(prospect.status) || ["replied", "booked", "completed", "suppressed"].includes(enrolment.status))) return NextResponse.json({ error: "Follow-up stopped because this person replied or is suppressed" }, { status: 400 });
    if (isReply && prospect.reply_category !== "interested") return NextResponse.json({ error: "This booking reply is no longer appropriate" }, { status: 400 });
    if (["suppressed", "not_interested"].includes(prospect.status) || enrolment.status === "suppressed") return NextResponse.json({ error: "This person is suppressed" }, { status: 400 });
    ...
    if (!isReply && (await activeClientDomains()).has(domain)) return NextResponse.json({ error: "This company already exists in the active CRM, outreach was blocked" }, { status: 400 });
    ...
    const sent = await sendOutreachMail({ to: email, subject: message.subject, text: message.body_text, threadId: message.gmail_thread_id || undefined });
    if (!sent.ok) {
      await Promise.all([
    ...
    const hasNext = !isReply && sequence.some((row: any) => Number(row?.step) === nextStep);
    ...
    await Promise.all([
      ...
      supabaseAdmin.from("outreach_enrolments").update({ status: isReply ? "replied" : hasNext ? "contacted" : "completed", current_step: isReply ? enrolment.current_step : hasNext ? nextStep : message.step_number, last_sent_at: now.toISOString(), next_action_at: nextAction, updated_at: now.toISOString() }).eq("id", enrolment.id),
      supabaseAdmin.from("outreach_prospects").update({ status: isReply ? "qualified" : "contacted", last_contacted_at: now.toISOString(), updated_at: now.toISOString() }).eq("id", prospect.id),
      supabaseAdmin.from("outreach_events").insert({ campaign_id: campaign.id, prospect_id: prospect.id, message_id: message.id, kind: "sent", metadata: { step: message.step_number, from: OUTREACH_FROM_EMAIL, messageType: isReply ? "reply" : "sequence", tags: message.message_tags || {} } }),
      ...(message.booking_link_included ? [supabaseAdmin.from("outreach_events").insert({ campaign_id: campaign.id, prospect_id: prospect.id, message_id: message.id, kind: "booking_link_shared", metadata: { step: message.step_number } })] : []),
    ]);
