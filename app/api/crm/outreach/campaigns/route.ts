...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("outreach_campaigns").insert({
      ...
      approval_mode: true,
      sequence: body.sequence || [{ step: 1, delayDays: 0, purpose: "Relevant opening" }, { step: 2, delayDays: 3, purpose: "Second use case" }, { step: 3, delayDays: 7, purpose: "Close the loop" }],
