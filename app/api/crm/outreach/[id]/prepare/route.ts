...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const senderGuidance =
      typeof body.guidance === "string" ? body.guidance.trim().slice(0, 1000) : "";
    ...
    const includeBooking = campaignCta?.delivery === "personal_booking_link"
      ? Boolean(campaignCta.url)
      : configuredCtaType !== "auto"
        ? false
        : shouldIncludePersonalOutreachBookingLink({
...
${originalText.slice(0, 9000) || "No usable formatted text was returned. Use only the safe context above and mark all prospect research as low confidence."}` }],
    ...
    const cleanedEmailBody = removeDashesFromProse(
      clean(parsed.email.bodyText, 4000)
    );
    const emailBody = campaignCta
      ? ensureOutreachEmailCampaignCta({
    ...
    const preparedVoiceScript = normaliseOutreachVoiceScript(
      prepareOutreachVoiceScriptForReview({
    ...
    const voiceScript = normaliseOutreachVoiceScript(
      campaignCta
        ? ensureOutreachVoiceCampaignCta({
            script: preparedVoiceScript,
            policy: campaignCta,
          })
        : configuredCtaType === "none"
          ? removeOutreachVoiceSalesCta(preparedVoiceScript)
        : preparedVoiceScript
    );
    ...
    const emailHasDemoReplyCta = campaignCta
      ? hasOutreachCampaignCta(email.body_text, campaignCta)
      : outreachEmailEndsWithDemoReplyCta(email.body_text);
    const voiceHasDemoReplyCta = campaignCta
      ? hasOutreachCampaignCta(voiceScript, campaignCta)
      : outreachVoiceEndsWithDemoReplyCta(voiceScript);
    ...
    const strategy = {
      ...
      cta: campaignCta?.label || clean(parsed.strategy.cta, 180),
