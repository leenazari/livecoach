...
type Tab = "queue" | "prospects" | "campaign" | "replies" | "safety";
...
export default function OutreachPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setMetrics(m.metrics || {}); setReplies(m.replies || []); setVariants(m.variants || []); setSuppressions(s.suppressions || []);
    ...
    <main className="relative z-10 mx-auto max-w-[1180px] px-3 py-5 sm:px-5 sm:py-9">
      ...
      {!loading && tab === "queue" ? <section>
        ...
        <div className="space-y-3">{queue.map((row, index) => { const p = row.prospect; const m = row.message; const edit = m ? draftEdits[m.id] || { subject: m.subject, body_text: m.body_text } : null; return <article key={row.id} style={{ contentVisibility: "auto" }} className="rounded-xl border border-edge bg-panel p-4">
          ...
          {row.research ? <details className="mt-4 rounded-lg border border-edge bg-ink/30 p-3"><summary className="cursor-pointer font-mono text-[0.6rem] uppercase tracking-wider text-amber">Why this message</summary><p className="mt-2 text-sm leading-6 text-bone/80">{row.research.summary}</p><p className="mt-2 text-xs text-muted"><strong className="text-bone">Best angle:</strong> {row.research.bestAngle}</p>{(row.research_sources || []).length ? <div className="mt-2 flex flex-wrap gap-2">{row.research_sources.slice(0, 4).map((source: any) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="text-xs text-amber hover:underline">{source.title || "Source"} ↗</a>)}</div> : null}</details> : null}
        ...
        </article>; })}{!queue.length ? <div className="rounded-xl border border-dashed border-edge p-8 text-center text-sm text-muted">Build today’s queue when you are ready. Nothing is selected or researched automatically.</div> : null}</div>
      </section> : null}
      ...
      {!loading && tab === "replies" ? <section><div className="mb-4 flex flex-col gap-2 rounded-xl border border-edge bg-panel p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-lg text-bone">Reply inbox</h2><p className="mt-1 text-sm text-muted">Positive replies are prioritised. Any reply immediately stops the sequence.</p></div><button onClick={checkReplies} disabled={!!busy} className={primary}>{busy === "replies" ? "Checking Gmail…" : "Check replies now"}</button></div><div className="space-y-2">{replies.map((reply) => <article key={reply.id} className="rounded-xl border border-edge bg-panel p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-display text-lg text-bone">{reply.first_name} {reply.last_name}</h3><p className="text-sm text-bone/80">{reply.company_name}</p></div><span className={`rounded-full border px-2 py-1 font-mono text-[0.55rem] uppercase ${reply.reply_category === "interested" ? "border-moss/50 text-moss" : "border-edge text-muted"}`}>{reply.reply_category}</span></div><p className="mt-3 text-sm leading-6 text-bone/80">{reply.reply_summary}</p><a href={`mailto:${reply.email}`} className="mt-3 inline-block font-mono text-xs text-amber">Reply from Gmail ↗</a></article>)}{!replies.length ? <div className="rounded-xl border border-dashed border-edge p-8 text-center text-sm text-muted">No replies detected yet.</div> : null}</div></section> : null}
