...
export default function OutreachPage() {
  ...
  const [sender, setSender] = useState<{ senderName: string; senderEmail: string; googleEmail: string } | null>(null);
  ...
  const approveAndSend = async (messageId: string) => {
    ...
    try {
      ...
      setNotice(
        `Exact draft approved and queued for ${formatActivityDate(result.scheduledAt)}. It will send automatically from ${sender?.senderEmail || "your connected Gmail"}.`
      );
  ...
  const rehearse = async (messageId: string) => {
    const rehearsalRecipient = sender?.googleEmail || "your connected Gmail";
    ...
    try {
      ...
      setQueue((all) => all.map((row) => row.message?.id === messageId ? { ...row, message: { ...row.message, ...message } } : row));
      const result = await crmFetch<{ ok: boolean; sentTo: string; from: string; campaignChanged: boolean }>(`/api/crm/outreach/messages/${messageId}/rehearse`, { method: "POST", body: "{}" });
      if (!result.ok || (sender?.googleEmail && result.sentTo !== sender.googleEmail) || result.campaignChanged !== false) throw new Error("The safe rehearsal was not confirmed");
      setNotice(`Rehearsal sent from ${result.from} to ${result.sentTo}. No prospect was contacted and campaign results did not change.`);
    }
    catch (e: any) { setError(e.message); } finally { setBusy(""); }
  };
  ...
  return (
    <main className="relative z-10 mx-auto max-w-[1180px] px-3 py-5 sm:px-5 sm:py-9">
      ...
      <header className="mb-4 flex items-start justify-between gap-3 border-b border-edge pb-4">
        <div><h1 className="font-display text-[1.55rem] tracking-tight text-bone"><span className="italic text-amber">Interviewa</span> outreach</h1><p className="mt-1 font-mono text-[0.57rem] uppercase tracking-wider text-muted">Approval mode · from {sender?.senderEmail || "your connected Gmail"} · maximum 20/day</p></div>
