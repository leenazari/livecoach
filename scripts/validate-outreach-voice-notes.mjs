...
assert.match(player, /senderEmail=\{message\.from_email\}/);
assert.match(player, /subject=\{message\.subject\}/);
...
assert.match(returnToInbox, /mailto:\$\{safeEmail\}/);
assert.match(returnToInbox, /window\.location\.assign\(mailtoHref\)/);
assert.match(returnToInbox, /navigator\.clipboard\.writeText\(safeEmail\)/);
assert.match(returnToInbox, /Open email reply/);
assert.match(returnToInbox, /Copy \{senderFirstName\}&apos;s email/);
assert.doesNotMatch(returnToInbox, /opened without inbox history/);
