...
function signingSecret(secret?: string): string {
  const value = String(secret || process.env.GOOGLE_CLIENT_SECRET || "").trim();
...
function normalizedReturnOrigin(value: string): string {
  ...
  if (url.protocol !== "https:" && !localDevelopment) {
    throw new Error("Google OAuth return origin must use HTTPS");
  }
  return url.origin;
}
...
export function createGoogleOAuthState(
  input: Omit<GoogleOAuthState, "issuedAt"> & { issuedAt?: number },
  secret?: string
): string {
  ...
  if (!payload.userId || !payload.workspaceId) {
    throw new Error("Google OAuth state is missing its account scope");
  }
...
export function verifyGoogleOAuthState(
  ...
  try {
    ...
    const payload = JSON.parse(decrypted) as Partial<GoogleOAuthStatePayload>;
    ...
    if (
      ...
      now - payload.issuedAt > GOOGLE_OAUTH_STATE_TTL_MS ||
    ...
    return {
      ...
      onboarding: payload.onboarding === true,
