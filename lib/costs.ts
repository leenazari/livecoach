...
export function claudeCallCostUSD(cachingWarm: boolean): number {
  const inUncached = TOKENS.transcriptWindow + TOKENS.instructions;
  const knowledgeCost = cachingWarm
    ? (TOKENS.knowledgeCached / 1_000_000) * RATES.haikuInPerM * RATES.haikuCacheReadPerM
    : (TOKENS.knowledgeCached / 1_000_000) * RATES.haikuInPerM;
...
export function estimateCost(
  ...
  // First call is a cache write, the rest are warm cache reads.
  let claude = 0;
  if (claudeCalls > 0) {
    const writeCost =
      (TOKENS.knowledgeCached / 1_000_000) *
        RATES.haikuInPerM *
    ...
    claude += writeCost;
    claude += (claudeCalls - 1) * claudeCallCostUSD(true);
  }
